...
-- ─────────────────────────────────────────────────────────────────
-- ================================================================
-- PinnacleQuiz — Safe Schema (run on existing OR new Supabase project)
-- Uses DROP IF EXISTS before creating, so it never fails on re-run
-- ================================================================
...
create policy "profiles_insert_own" on public.profiles for insert with check (auth.uid() = id or auth.uid() is not null);
...
create policy "sessions_manage" on public.game_sessions for update using (auth.uid() = host_id);
...
-- ── 4. REALTIME (safe — only adds if not already a member) ─────
